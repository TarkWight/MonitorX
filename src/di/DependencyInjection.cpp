...
{
    m_logger = std::make_unique<LogManager>(QCoreApplication::applicationDirPath() + "/monitor.log", parent);

    m_hasher = std::make_unique<HashManager>(m_logger.get());

    m_backupManager = std::make_unique<BackupManager>(m_logger.get(), parent);

    m_configManager = std::make_unique<ConfigManager>(configPath, m_logger.get(), parent);
