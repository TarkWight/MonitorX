//===-- MonitorX/src/ui/MainWindow.cpp - MonitorX Main UI Implementation --*- C++ -*-===//
//
//                     MonitorX — Save File Monitoring Tool
//
// This file defines the logic for binding UI controls to the view model,
// managing button states, directory selection, and reacting to signals.
//
//===--------------------------------------------------------------------------------===//
...
void MainWindow::setViewModel(MonitorViewModel *vm) {
    ...
    connect(ui->buttonBrowse, &QPushButton::clicked, this, [this]() {
        const QString dir = QFileDialog::getExistingDirectory(
            this, tr("Выберите папку с сохранениями"), m_vm->watchDirectory());
        if (!dir.isEmpty()) {
            m_vm->setWatchDirectory(dir);
        }
    });

    connect(ui->buttonStart, &QPushButton::clicked, vm, &MonitorViewModel::start);
    connect(ui->buttonStop, &QPushButton::clicked, vm, &MonitorViewModel::stop);
    connect(ui->buttonLogs, &QPushButton::clicked, vm, &MonitorViewModel::openLogs);
...
void MainWindow::setRunning(bool running) {
    ui->frameRunIndicator->setStyleSheet(QString(
                                             ...
                                             .arg(running ? "lightblue" : "darkgreen"));
